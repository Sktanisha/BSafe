...
//    onResume(), onPause(), onStop(), onRestart(), onDestroy()
    ...
    protected void onResume() {
        ...
        ArrayList<Feedbacks> items = new ArrayList<>();

        UserFeedbackTable db = new UserFeedbackTable(this);
        Cursor rows = db.execute("SELECT * FROM key_value_pairs");

        while(rows.moveToNext()){
            String Feedbacks = rows.getString(0);     // returns the first column out of the 2 columns
            ...
            items.add(new Feedbacks(Feedback_id,fdbk,comment));
        }
